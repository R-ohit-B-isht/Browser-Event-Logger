...
export default class BaseGenerator {
  ...
  _handleClick(selector, events) {
    ...
    const { text, id, name, dataAttributes } = events
  
    // Build a more specific selector if additional information is available
    let specificSelector = selector
    if (text) {
      specificSelector += `:has-text("${this._escapeUserInput(text)}")`
    }
    if (id) {
      specificSelector += `#${id}`
    }
    if (name) {
      specificSelector += `[name="${name}"]`
    }
    if (dataAttributes) {
      Object.entries(dataAttributes).forEach(([key, value]) => {
        specificSelector += `[${key}="${value}"]`
      })
    }
  
    block.addLine({
      type: eventsToRecord.CLICK,
      value: `await ${this._frame}.click('${specificSelector}')`,
    })
  
    // Optionally, add a comment with the original selector and additional info for clarity
    block.addLine({
      type: 'comment',
      value: `// Original selector: ${selector}`,
    })
    if (text) {
      block.addLine({
        type: 'comment',
